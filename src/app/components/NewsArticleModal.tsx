        ...
        >
          ...
          >
            ...
            >
              ...
              <div
                className={`p-4 sm:p-5 rounded-2xl border ${READER_CARD_THEME_CLASSES[readerTheme]} shadow-xs`}
              >
                <div className="flex items-center gap-2 mb-2 text-[var(--brand-green)] font-bold text-sm">
                  <BookOpen className="w-4 h-4" />
                  <span>الموجز التنفيذي:</span>
                </div>
                <p className="m-0 font-medium">{article.excerpt}</p>
              </div>
              ...
              {article.content ? (
                ...
                <>
                  ...
                  <h2 className="font-bold text-xl sm:text-2xl pt-2">
                    أبرز محاور المبادرة والأبعاد الإنسانية
                  </h2>
                  <p>
                    تواصل حملة رحماء بينهم تنفيذ برامجها الاستراتيجية المستدامة بهدف تعزيز التكافل
                    الاجتماعي ورفع المعاناة عن الأسر المتضررة في المحافظات والمناطق النائية. تأتي
                    هذه المبادرة امتداداً للرؤية الإنسانية المتكاملة التي تسعى لبناء مجتمع كافٍ
                    ومتعلم ويمتلك مقومات الحياة الكريمة.
                  </p>
                  <p>
                    وقد تضمنت هذه المرحلة دراسات ميدانية مسحية شاملة شملت الفرق الهندسية والفرق
                    الاجتماعية لتقييم الاحتياجات الدقيقة، وضمان وصول الدعم المباشر إلى مستحقيه بكل
                    شفافية وعدالة وفق المعايير المعتمدة دولياً ومحلياً.
                  </p>

                  {/* Quote Pullout */}
                  <div
                    className={`p-5 rounded-2xl border-r-4 my-6 ${READER_CARD_THEME_CLASSES[readerTheme]}`}
                  >
                    <Quote className="w-7 h-7 text-[#C69E5A] mb-2 opacity-80" />
                    <p className="italic font-medium">
                      « إن العمل الإنساني الشامل لا يقتصر على تقديم العون المؤقت، بل يكمن في تمكين
                      الأسر وبناء قدرات الشباب وتحويل الحاجة إلى طاقات إنتاجية مستدامة تثمر خيراً
                      على المجتمع بأكمله. »
                    </p>
                    <div className="text-xs opacity-75 mt-2 font-bold">
                      <span>— من كلمة الإدارة التنفيذية لحملة رحماء بينهم</span>
                    </div>
                  </div>

                  {/* Section: Numbers & Impact */}
                  <h2 className="font-bold text-xl sm:text-2xl pt-2">
                    الأثر الميداني والأرقام المحققة
                  </h2>
                  <p>
                    بفضل الدعم الكريم والمستمر من أهل الخير والشُركاء الداعمين، حققت المبادرة نتائج
                    ملموسة انعكست مباشرة على حياة آلاف المستفيدين، ويمكن تلخيص أبرز الثمار والمؤشرات
                    الميدانية في النقاط التالية:
                  </p>

                  <ul className="list-disc list-inside space-y-2 pr-2">
                    <li>
                      تغطية أكثر من <strong>عشرات المديريات والقرى أشد احتياجاً</strong> وتأمين
                      احتياجاتهم المباشرة.
                    </li>
                    <li>
                      توزيع الحقائب المدرسية والمستلزمات التعليمية على أكثر من{" "}
                      <strong>آلاف الطلاب والطالبات</strong>.
                    </li>
                    <li>
                      تقديم المنح والتسهيلات التمويلية لدعم أكثر من{" "}
                      <strong>مشاريع صغيرة مدعومة للأسر المنتجة</strong>.
                    </li>
                    <li>
                      تأهيل وتشغيل <strong>آبار مُعاد تأهيلها</strong> مياه صحية تعمل بالطاقة
                      الشمسية المستدامة.
                    </li>
                  </ul>
                  ...
                  <div
                    className={`p-5 rounded-2xl border ${READER_CARD_THEME_CLASSES[readerTheme]}`}
                  >
                    ...
                    <p className="m-0 text-sm opacity-90">
                      تلتزم حملة رحماء بينهم بنشر كافة التقارير الميدانية والمالية الخاصة بالمشاريع
                      بشكل دوري، مع إتاحة التتبع المباشر للتبرعات لضمان أقصى درجات النزاهة
                      والمصداقية تجاه جميع الداعمين.
                    </p>
                  </div>

                  <p>
                    ونحن إذ نشكر جميع أيادي الخير والشُركاء الذين ساهموا في إنجاح هذا العمل، نؤكد
                    استمرار قوافل الخير وبرامج التمكين في المستقبل القريب لتشمل مناطق جديدة وأسر
                    أخرى تنتظر البصمة الإنسانية المضيئة.
                  </p>
                </>
              )}
            </article>
            ...
            {article.tags && article.tags.length > 0 && (
              <div className="pt-6 border-t border-slate-200/30 flex flex-wrap items-center gap-2 max-w-3xl mx-auto">
                ...
                {article.tags.map((tag) => (
                  <span
                    key={tag}
                    className="px-3 py-1 rounded-full text-xs font-semibold bg-slate-500/10 hover:bg-slate-500/20 transition-colors"
                  >
                    #{tag}
                  </span>
                ))}
            ...
            <div className="p-6 sm:p-7 rounded-2xl bg-gradient-to-l from-[var(--brand-green)] to-emerald-800 text-white shadow-lg flex flex-col sm:flex-row items-center justify-between gap-5 my-8 max-w-3xl mx-auto">
              <div className="space-y-1 text-center sm:text-right">
                ...
                <p className="text-xs sm:text-sm text-slate-100 max-w-md">
                  تبرعك البسيط اليوم يساهم في مواصلة هذه البرامج واستدامة الخير لأسر ومستفيدين
                  آخرين.
                </p>
              </div>
