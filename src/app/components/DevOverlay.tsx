          ...
          >
            ...
            <div className="px-4 py-3 bg-slate-900/90 border-b border-slate-800/80 flex items-center justify-between">
              <div className="flex items-center gap-2">
                ...
                <span className="font-bold text-slate-200 text-xs tracking-wide">Developer Telemetry</span>
            ...
            <div className="grid grid-cols-3 bg-slate-900/40 border-b border-slate-800/80 p-2.5 text-center gap-2">
              <div className="p-2 rounded bg-slate-900/80 border border-slate-800">
                ...
                <div className={`text-base font-bold ${fps >= 50 ? 'text-emerald-400' : fps >= 30 ? 'text-amber-400' : 'text-rose-400'}`}>
                  {fps}
                </div>
              </div>
                ...
                <div className="space-y-3">
                  <div className="space-y-1">
                    <div className="flex justify-between text-[11px] text-slate-300">
                      ...
                      <span className="font-bold text-cyan-400">{usedHeap} MB / {totalHeap} MB</span>
                    </div>
                    <div className="w-full h-2 rounded-full bg-slate-800 overflow-hidden">
                      <div 
                        className="h-full bg-gradient-to-r from-emerald-500 to-cyan-400 transition-all duration-500"
                        style={{ width: `${totalHeap ? Math.min((usedHeap / totalHeap) * 100, 100) : 0}%` }}
                      />
                    </div>
                  </div>
                  ...
                  <div className="grid grid-cols-2 gap-2 text-[11px]">
                    <div className="p-2 rounded bg-slate-900 border border-slate-800">
                      ...
                      <div className="text-slate-200 font-bold mt-0.5">{heapLimit ? `${heapLimit} MB` : 'Browser Restricted'}</div>
                    </div>
                    <div className="p-2 rounded bg-slate-900 border border-slate-800">
                ...
                <div className="space-y-3">
                  <div className="grid grid-cols-3 gap-2 text-[11px]">
                    ...
                    <div className="p-2 rounded bg-slate-900 border border-slate-800 text-center">
                      ...
                      <div className="text-slate-200 font-bold text-xs mt-0.5">{minFps} / {maxFps}</div>
                    </div>
                    <div className="p-2 rounded bg-slate-900 border border-slate-800 text-center">
              ...
              {activeTab === 'network' && (
                <div className="space-y-2">
                  <div className="flex items-center justify-between pb-1 border-b border-slate-800">
                    <span className="text-[11px] text-slate-400">Recent Resources ({networkLogs.length})</span>
                    <button
                  ...
                  <div className="space-y-1.5 max-h-[180px] overflow-y-auto pr-1">
                    {networkLogs.length === 0 ? (
                      <div className="text-center py-4 text-slate-500 text-[11px]">No network timing entries recorded yet.</div>
                    ) : (
                      networkLogs.map((item, idx) => (
                        <div key={idx} className="p-2 rounded bg-slate-900 border border-slate-800/80 flex items-center justify-between text-[10px]">
                          <div className="truncate max-w-[180px]" title={item.name}>
                            <div className="text-slate-200 font-bold truncate">{item.name}</div>
                            <div className="text-slate-500 text-[9px] uppercase">{item.type} • {item.protocol}</div>
                          </div>
                          <div className="text-right shrink-0">
                            <div className="text-emerald-400 font-bold">{item.duration} ms</div>
                            <div className="text-slate-400 text-[9px]">{item.transferSize ? `${item.transferSize} KB` : 'Cached'}</div>
                          </div>
                        </div>
                      ))
                    )}
