      ...
      try {
        ...
        setAnalytics({
          ...
          trafficSources: [
            {
              source: "مباشر",
              visitors: Math.floor(totalInteractions * 0.45),
              percentage: 45,
              color: "var(--brand-green)",
            },
            {
              source: "بحث Google",
              visitors: Math.floor(totalInteractions * 0.3),
              percentage: 30,
              color: "#3B82F6",
            },
            {
              source: "وسائل التواصل",
              visitors: Math.floor(totalInteractions * 0.15),
              percentage: 15,
              color: "#F59E0B",
            },
            {
              source: "روابط داخلية",
              visitors: Math.floor(totalInteractions * 0.1),
              percentage: 10,
              color: "#8B5CF6",
            },
          ],
            ...
            {analytics.topPages.map((page, i) => (
              <div key={page.path} className="flex items-center justify-between">
                <div className="flex items-center gap-2">
                  <div
                    className="w-6 h-6 rounded-full bg-[var(--brand-green-pale)] flex items-center justify-center text-[var(--brand-green)]"
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-xs ${page.change >= 0 ? "text-[var(--brand-green)]" : "text-red-500"}`}
                  >
                    {page.change >= 0 ? "+" : ""}
                    {page.change}%
                  </span>
              ...
              return (
                <div key={stat.date} className="flex flex-col items-center flex-1 max-w-12">
                  <div className="relative w-full">
                    <div
                      className="absolute -top-5 left-1/2 -translate-x-1/2 text-[var(--muted-foreground)]"
                      style={{ fontSize: "0.6rem" }}
                    >
                      {stat.views}
                    </div>
                    <div
                      className="w-full bg-[var(--brand-green)] rounded-t transition-all hover:bg-[var(--brand-green-light)]"
            ...
            <div className="relative w-32 h-32">
              <svg className="w-full h-full transform -rotate-90">
                {analytics.deviceStats.map((device, i) => {
                  ...
                  const offset = analytics.deviceStats
                    .slice(0, i)
                    .reduce((sum, d) => sum + (d.percentage / total) * 100, 0);
                  return (
                    <circle
                      key={device.device}
                      cx="64"
  ...
  return (
    <div className="bg-white rounded-xl p-5 border border-[var(--border)]">
      <div className="flex items-start justify-between mb-3">
        ...
        {trend && (
          <span
            className="px-2 py-0.5 rounded-full bg-[var(--success-bg)] text-[var(--success)]"
