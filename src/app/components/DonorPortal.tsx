              ...
              <div className="bg-white rounded-3xl p-6 shadow-lg border border-gray-100">
                ...
                <div className="space-y-4">
                  {[
                    ...
                    <div key={campaign.name} className="p-4 bg-gradient-to-l from-emerald-50 to-white rounded-xl border border-[var(--success)]">
                      <div className="flex items-center justify-between mb-2">
                          ...
                          {donation.impact && (
                            <div className="flex flex-wrap gap-2 mt-2">
                               {donation.impact.map((imp, _idx) => (
                                 <span key={imp.metric} className="text-xs bg-[var(--success-bg)] text-[var(--success)] px-2 py-1 rounded-lg flex items-center gap-1">
            ...
            <motion.div key="notifications" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -20 }}>
              <div className="bg-white rounded-3xl p-6 shadow-lg border border-gray-100">
                <h3 className="text-lg font-bold text-gray-800 mb-6 flex items-center gap-2">
                  ...
                  الإشعارات
                  {activeNotifications > 0 && (
                    <span className="px-2 py-0.5 bg-red-100 text-[var(--destructive)] text-xs font-bold rounded-full">{activeNotifications} جديد</span>
                  )}
                </h3>
                <div className="space-y-3">
                  {DEFAULT_NOTIFICATIONS.map(notif => (
                    <div
                      ...
                      className={`flex items-start gap-4 p-4 rounded-xl transition-all ${
                        notif.read ? 'bg-white hover:bg-gray-50' : 'bg-[var(--success-bg)] border border-[var(--success)]'
                      }`}
                    >
                      ...
                      <div className="flex-1">
                        <div className="flex items-center justify-between mb-1">
                          ...
                          {!notif.read && <span className="w-2 h-2 bg-[var(--success-bg)]0 rounded-full" />}
                        </div>
