...
const ProfessionalVideoPlayer = memo(
  ({
    ...
    const getPlayerSizeClasses = () =>
      playerSize === "large" ? "max-w-6xl scale-105" : "max-w-5xl";
            ...
            <div className="flex items-start justify-between">
              <div className="flex-1 text-white">
                ...
                <div className="flex items-center gap-3 text-sm text-white/70">
                  ...
                  <span>
                    {video.publishDate
                      ? new Date(video.publishDate).toLocaleDateString("ar-YE")
                      : "—"}
                  </span>
                </div>
              </div>
          ...
          <div className="p-6 bg-white dark:bg-gray-900">
            ...
            <div className="flex flex-wrap items-center gap-4 mt-3 text-sm text-[var(--muted-foreground)]">
              ...
              <span className="flex items-center gap-1">
                ...
                {video.likes?.toLocaleString()} إعجاب
              </span>
              {video.category && (
                <span className="bg-[var(--secondary)] px-3 py-1 rounded-full text-xs">
                  {video.category}
                </span>
              )}
            </div>
    ...
    return (
      <motion.div
        ...
        className={`cursor-pointer rounded-xl overflow-hidden border-2 transition-all ${
          isActive
            ? "border-[var(--brand-green)] shadow-xl shadow-[var(--brand-green)]/20"
            : "border-transparent hover:border-[var(--brand-green)]/40"
        }`}
        onClick={onClick}
      ...
      >
        ...
        <div className="p-4 bg-white dark:bg-gray-800">
          ...
          <div className="flex items-center justify-between mt-2 text-xs text-[var(--muted-foreground)]">
            ...
            {video.publishDate && (
              <span className="bg-[var(--secondary)] px-2 py-0.5 rounded-full text-[10px]">
                {new Date(video.publishDate).toLocaleDateString("ar-YE")}
              </span>
            )}
          </div>
...
export const VideoShowcase = memo(
  ({
    ...
    const [youtubeChannelUrl, setYoutubeChannelUrl] = useState<string>(
      "https://www.youtube.com/@RahmaaBenahum"
    );
    ...
    useEffect(() => {
      const fetchData = async () => {
        try {
          // جلب الفيديوهات
          const fetchedVideos =
            initialVideos || (await sanityClient.fetch<VideoItem[]>(VIDEOS_QUERY));
