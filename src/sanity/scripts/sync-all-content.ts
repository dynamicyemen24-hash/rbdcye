      ...
      for (const item of SEED_SUCCESS_STORIES) {
        try {
          const existing = await sanityClient.fetch(
            `*[_type == "successStory" && _id == "${item.id}"][0]`
          );
    ...
    try {
      for (const item of SEED_PARTNERS) {
        try {
          ...
          const imageRef = item.logo
            ? await this.uploadImage(item.logo, item.name, `partner-${item.id}`)
            : undefined;
